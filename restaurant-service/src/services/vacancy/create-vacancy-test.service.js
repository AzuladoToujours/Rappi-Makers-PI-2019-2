...
const {
  createVacancyTestFetch
} = require('../../fetchs/vacancy/create-vacancy-test-fetch');
...
const createVacancyTestService = async (req, res) => {
  const token = req.headers.authorization.split(' ')[1];
  const response = await createVacancyTestFetch(token, req.body);
...
module.exports = { createVacancyTestService };
