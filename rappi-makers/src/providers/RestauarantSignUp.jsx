...
import { SignUp, EditRestaurant } from '../services/Restaurant';
...
const RestaurantSignUpProvider = ({ children }) => {
  ...
  const editRestaurant = async (values) => {
    setLoading(true);
    const response = await EditRestaurant(values, token);
    const { error } = response;
    if (!error) {
  ...
  return (
    <RestaurantSignUpContext.Provider
      value={{ formSubmit, editRestaurant, onClose, open, error, errorMessage, title, message, loading }}
    >
      {children}
    </RestaurantSignUpContext.Provider>
  );
